 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (uint t = 1; t < N; t++) {
      ...
      AD<double> psides0 = CppAD::atan(coeffs[1] + 2 * coeffs[2] * x0 + 3 * coeffs[3] * CppAD::pow(x0, 2)); // arctan(f'(x))
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  for (size_t i = 0; i < N-1; i++) {
    res.push_back(solution.x[1 + x_start + i]);
    res.push_back(solution.x[1 + y_start + i]);
  }
  return res;
}
